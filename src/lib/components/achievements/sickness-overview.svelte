<script lang="ts">
	...
	const rate = $derived(workdays > 0 ? Math.round((sickAllTime / workdays) * 100) : 0);
</script>
	...
	<Card.Content class="pb-5">
		<div class="grid grid-cols-3 gap-3 text-center">
			<div>
				...
				<p class="text-[11px] text-muted-foreground">{m.ach_sick_this_year()}</p>
			</div>
			<div>
				<p class="text-2xl font-bold tabular-nums">{sickAllTime}</p>
				<p class="text-[11px] text-muted-foreground">{m.ach_sick_all_time()}</p>
			</div>
			<div>
				<p class="text-2xl font-bold tabular-nums">{rate}%</p>
				<p class="text-[11px] text-muted-foreground">
					{m.ach_sick_rate({ sick: sickAllTime, work: workdays })}
				</p>
			</div>
		</div>
	</Card.Content>
